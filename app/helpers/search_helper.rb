module SearchHelper
  ...
  def format_homologies(list)
    list.collect do |item|
      ...
      %Q|<a href="/search/entity/#{entity_id}" title="#{entity_id}">#{entity_name}</a> in <a href="/search/taxon/#{taxon_id}" title="#{taxon_id}">#{taxon_name}</a> <a href="#" onclick="showSource('#{entity_id}', '#{taxon_id}', this); return false;">[Source]</a>|
		end
  end
